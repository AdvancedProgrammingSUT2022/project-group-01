...
public class UnitController {
	...
	public String meleeAttack(Armed armed, Tile tile) {
		...
		if(tile.getInnerCity() == null && tile.getArmedUnit() != null){
			...
			if(armed.getTraitsList().contains(UnitTraits.BONUS_VS_TANK) && target.getType().equals(UnitType.TANK))
				unitAttackModifier *= 1.5f;
			if(armed.getTraitsList().contains(UnitTraits.BONUS_VS_MOUNTED) && target.getType().getCombatType().equals(CombatType.MOUNTED))
				unitAttackModifier *= 1.5f;
		...
		if(tile.getInnerCity().getState().equals(CityState.NORMAL))
