...
public class DemocracyMainPanel {
    ...
    private void addChosenCivMenu(Civilization c){
        ...
        Label civ = new Label();
        String nameDataCiv = chosenCivilization.getCivilization().getName() + " --> state : ";
        if(gmp.getGameMenuController().getGame().getCurrentPlayer().getCivilization().isInWarWith(chosenCivilization))
            nameDataCiv += "in war";
        else
            nameDataCiv += "in peace";
        civ.setText(nameDataCiv);
        civ.setTextFill(Color.WHITESMOKE);
        civ.setFont(Font.font("AppleGothic Regular",20));
        civ.setLayoutX(5);
        civ.setLayoutY(10);
        mainPane.getChildren().add(civ);
        ...
        war.setOnMouseClicked(e -> {
            ...
            data.put("player",chosenCivilization.getPlayer().getUser().getUsername());
            new PopUp().run(PopUpStates.OK,gmp.getGameMenuController().declareWar(data));
        });
        ...
        peace.setOnMouseClicked(e -> {
            gmp.getGameMenuController().getGame().getCurrentPlayer().getCivilization().makePeaceWith(chosenCivilization);
        });
