...
//	}
	...
	public String register(HashMap<String, String> args) {
		...
		request.addData("args", new Object[]{args.get("username"), args.get("password"), args.get("nickname"), ImagesAddress.AVATAR1.name()});
		System.out.println("ghabl response" + request.toXML());
		Response response = NetworkController.send(request);
		return response.getMessage();
	}
