...
public class Unit implements Identifiable {
	...
	public Unit(UnitType type, Tile tile, Civilization civilization) {
		...
		this.game = ClientHandler.game.get();
	...
	private Vector<Tile> dijkstra(Tile destination) {
		lastDijkstraRemMP = -1;
		Map gameMap = this.game.getMap();//TODO get user map in next checkpoint
...
//				System.err.printf("^^ (%d, %d)\n", nextDistance.turn, nextDistance.remainedMP);
	...
	private class Distance implements Comparable<Distance> {
		...
		public Distance getDistanceAfter(Tile currentTile, Boarder boarder) {
			...
			if (currentTile.getGame().getCurrentPlayer().getSavedMap().getVisibilityState(nextTile).equals(Tile.VisibilityState.FOG_OF_WAR))
	...
	protected static void updateMapAfterMove(Game game) {
