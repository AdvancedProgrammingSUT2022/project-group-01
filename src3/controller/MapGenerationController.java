...
public class MapGenerationController extends Controller{
    ...
    private double[][] makeNoise(OpenSimplexNoise noise ,int mapSize){
        ...
        for (int y = 0; y < mapSize; y++) {
            for (int x = 0; x < mapSize; x++) {
                ...
                array[y][x] =    1 * noise.eval(1 * nx, 1 * ny)
                        +  0.5 * noise.eval(2 * nx, 2 * ny)
                        + 0.25 * noise.eval(4 * nx, 4 * ny);
    ...
    private TerrainFeature getFeatureByHumidity(double noise, Tile tile){
        ...
        switch (terrain){
            case DESERT -> {
                if(noise < 0.3)
                    return TerrainFeature.OASIS;
                if(noise > 0.3) {
                    if(tile.hasRiverNearby())
                        return TerrainFeature.FLOOD_PLAINS;
                }
                return null;
            }
            case GRASSLAND -> {
    ...
    private void setBoardSide(int p, int q, int nextP, int nextQ, int i, Tile[][] board, int mapSize){
        ...
        if ((nextP < 0) || (nextP > mapSize - 1) || (nextQ < 0) || (nextQ > mapSize - 1) || (nextP + nextQ < (mapSize - 1) / 2) || (nextP + nextQ > 3 * (mapSize - 1) / 2)) {
            ...
            if(board[q][p].getBoarder(i) != null)
                return;
    ...
    private void setRiverRoots(Tile[][] board){
        ...
        for(int q = 0; q < game.getMap().getMapSize(); q++){
            for(int p = 0; p < game.getMap().getMapSize(); p++){
                if(board[q][p] != null){
                    if(board[q][p].getTerrain().equals(Terrain.MOUNTAIN))
                        mountains.add(board[q][p]);
                }
            }
        }
        ...
        for(Tile tile : mountains){
            if(rand.nextInt() % 2 == 0)
                continue;
            addRivers(tile.getBoarder(rand.nextInt() % 6));
        }
    }
    ...
    private void getContinent(Tile[][] board){
        ...
        for(int q = 0; q < game.getMap().getMapSize(); q++){
            for(int p = 0; p < game.getMap().getMapSize(); p++){
                ...
                if(numberOfResources <= 0)
                    continue;
                int resourceIndex = rand.nextInt(numberOfResources);
