...
public class MapController  extends Controller {
    ...
    public String setPosition(int x){
        ...
        if((x < 0) | (x >= mapSize * mapSize))
            return "invalid position";
        Tile mysteryTile;
        if((mysteryTile = game.getMap().getTileByNumber(x)) == null)
            return "invalid position";
    ...
    public String setMapOnCity(String cityName){
        ...
        City city;
        if((city = checkCityValidity(cityName)) == null)
            return "you dog. There is no city with this name.";
...
//    }
    ...
    public void setMap(Tile centerTile, ConsoleMap showingMap, Player player, Map map){
        ...
        for(int q = centerTile.getQCoordinate() - 2; q < centerTile.getQCoordinate() + 3; q++){
            for(int p = centerTile.getPCoordinate() - 2; p < centerTile.getPCoordinate() + 3; p++){
                ...
                if(p + q < minSum)
                    continue;
                if(Math.abs(p + q - centerTile.getPCoordinate() - centerTile.getQCoordinate()) > 2)
                    continue;
    ...
    private void setBoarder(Tile tile, ConsoleMap showMap, int xCoordinate, int yCoordinate){
        ...
        if(isRiver)
            boarder = ConsoleMap.getRepresentation("_R");
        ...
        if(isRiver)
            boarder = boarder = ConsoleMap.getRepresentation("_R");
        for(int x = xCoordinate - 3; x < xCoordinate + 4; x++){
            showMap.setScreenMapIndex(x, yCoordinate + 3, boarder);
        }
        // set sides
        ...
        if(tile.getBoarderInfo(5).isRiver())
            boarder = boarder = ConsoleMap.getRepresentation("/R");
        for(int i = 0; i < 3; i++){
